...
export default function CreativeSection() {
    ...
    uiux: [
      {
        ...
        desc: "Modular Figma & code component tokens ensuring 100% visual consistency and WCAG 2.1 AA accessibility standards.",
  ...
  return (
    <section id="design" className="relative py-28 px-6 md:px-12 bg-black border-t border-white/10">
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col space-y-4 max-w-2xl mb-16">
          ...
          <p className="text-neutral-400 text-sm sm:text-base leading-relaxed">
            Uniting deep full-stack engineering logic with high-end graphic design, UI/UX wireframing,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {creativeItems[activeTab].map((item, idx) => (
            <motion.div
              ...
              className="p-7 rounded-2xl bg-gradient-to-b from-[#111111] via-[#0A0A0A] to-[#060606] border border-white/10 hover:border-[#A100FF]/60 transition-all flex flex-col justify-between group shadow-xl"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between pb-3 border-b border-white/5">
                  <span className="font-mono text-[10px] text-[#A100FF] uppercase tracking-widest">
                    DISCIPLINE 0{idx + 1}
                  </span>
                  <div className="w-2 h-2 rounded-full bg-[#A100FF]/40 group-hover:bg-[#A100FF] transition-colors" />
                </div>

                <h3 className="text-lg font-bold text-white uppercase tracking-tight group-hover:text-[#C084FC] transition-colors">
                  {item.title}
                </h3>
              ...
              <div className="pt-6 mt-6 border-t border-white/5 flex flex-wrap gap-1.5">
                {item.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2 py-0.5 rounded bg-white/5 font-mono text-[10px] text-neutral-300"
                  >
                    {tag}
                  </span>
                ))}
